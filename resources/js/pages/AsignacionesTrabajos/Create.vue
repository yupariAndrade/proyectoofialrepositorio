    ...
    <AppContent>
      <div class="mb-8 bg-gradient-to-r from-slate-800/50 to-slate-900/50 backdrop-blur-lg border-b border-white/10">
        <div class="px-8 py-6 flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-cyan-300 via-sky-400 to-indigo-400 bg-clip-text text-transparent">Nueva Asignación</h1>
            <p class="text-slate-400">Asigna un usuario a un trabajo</p>
          </div>
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input v-model="form.turno" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white focus:ring-2 focus:ring-cyan-500" placeholder="Mañana, Tarde, Noche" />
            ...
            <div class="flex justify-end gap-4 pt-6 border-t border-white/10">
              ...
              <button type="submit" :disabled="form.processing" class="px-6 py-3 bg-gradient-to-r from-cyan-500 to-indigo-600 text-white rounded-lg">{{ form.processing ? 'Guardando...' : 'Guardar' }}</button>
            </div>
          </form>
        </div>
      </div>
