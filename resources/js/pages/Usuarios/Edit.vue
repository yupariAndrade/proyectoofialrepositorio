          ...
          <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/10 p-8">
            <form @submit.prevent="actualizarUsuario" class="space-y-8">
              ...
              <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input v-model="form.nombre" type="text" required class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                  <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Apellido Paterno</label>
                    <input v-model="form.apellidoPaterno" type="text" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                  <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Apellido Materno</label>
                    <input v-model="form.apellidoMaterno" type="text" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                  <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">CI</label>
                    <input v-model="form.ci" type="text" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input v-model="form.email" type="email" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                  <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Teléfono</label>
                    <input v-model="form.telefono" type="tel" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                  <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Dirección</label>
                    <input v-model="form.direccion" type="text" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input v-model="form.fechaIngreso" type="date" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white" />
                  </div>
                  <div>
              ...
              <div class="flex items-center justify-end pt-8 border-t border-slate-600/50">
                ...
                <button type="submit" class="ml-4 px-8 py-3 bg-gradient-to-r from-amber-500 via-pink-500 to-purple-600 hover:from-amber-600 hover:via-pink-600 hover:to-purple-700 text-white rounded-xl">Actualizar Usuario</button>
              </div>
            </form>
          </div>
        </div>
...
const props = defineProps({ usuario: { type: Object, required: true }, roles: { type: Array, required: true } })

const form = ref({ nombre: '', apellidoPaterno: '', apellidoMaterno: '', ci: '', telefono: '', direccion: '', email: '', fechaIngreso: '', fechaFinal: '', estado: true, idRol: '' })

onMounted(() => {
  const u = props.usuario || {}
  form.value = { nombre: u.nombre || '', apellidoPaterno: u.apellidoPaterno || '', apellidoMaterno: u.apellidoMaterno || '', ci: u.ci || '', telefono: u.telefono || '', direccion: u.direccion || '', email: u.email || '', fechaIngreso: u.fechaIngreso || '', fechaFinal: u.fechaFinal || '', estado: Boolean(u.estado), idRol: u.idRol || '' }
})

const actualizarUsuario = () => { if (!form.value.nombre || !form.value.idRol) return alert('Nombre y Rol son obligatorios'); router.put(route('usuarios.update', props.usuario.id), form.value, { onSuccess: () => router.visit(route('usuarios')) }) }
</script>
