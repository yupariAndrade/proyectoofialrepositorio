    ...
    <AppContent>
      ...
      <div class="mb-8 bg-gradient-to-r from-slate-800/50 to-slate-900/50 backdrop-blur-lg border-b border-white/10">
        <div class="px-8 py-6 flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-cyan-300 via-sky-400 to-indigo-400 bg-clip-text text-transparent">Editar Pago</h1>
            <p class="text-slate-400">Modifica la información del pago</p>
          </div>
        ...
        <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-lg rounded-2xl border border-white/10 shadow-2xl p-8">
          <form @submit.prevent="submit" class="space-y-8">
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">Trabajo *</label>
                <select v-model="form.idTrabajo" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent" required>
              ...
              <div>
                ...
                <select v-model="form.idEstadoPago" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent" required>
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <div class="relative">
                  ...
                  <input v-model="form.monto" type="number" step="0.01" min="0" class="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-cyan-500 focus:border-transparent" placeholder="0.00" required />
                </div>
              ...
              <div>
                ...
                <input v-model="form.fecha" type="date" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent" required />
                <div v-if="form.errors.fecha" class="text-red-400 text-sm mt-1">{{ form.errors.fecha }}</div>
              </div>
            </div>

            <div>
              <label class="block text-sm font-medium text-slate-300 mb-2">Comentario</label>
              <textarea v-model="form.comentario" rows="3" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-cyan-500 focus:border-transparent" placeholder="Comentario opcional (máx. 255)"></textarea>
...
const props = defineProps({ pago: { type: Object, required: true }, trabajos: { type: Array, required: true }, estadosPago: { type: Array, required: true } })

const form = useForm({
  idTrabajo: props.pago.idTrabajo || '',
  idEstadoPago: props.pago.idEstadoPago || '',
  monto: props.pago.monto || '',
  fecha: props.pago.fecha || '',
  comentario: props.pago.comentario || ''
})

const submit = () => { form.put(route('pagos.update', props.pago.idPago)) }
</script>
