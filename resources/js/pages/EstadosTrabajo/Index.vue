        ...
        <AppContent>
            <div class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black flex-1">
                ...
                <main class="flex-1 overflow-y-auto p-8">
                    ...
                    <!-- Lista de estados -->
                    <div v-if="filteredEstados.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                        <div
                            v-for="estado in filteredEstados"
                            :key="estado.id"
                            class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-lg rounded-xl shadow-2xl border border-white/10 overflow-hidden hover:shadow-amber-500/20 transition-all duration-300 transform hover:-translate-y-2 hover:scale-105"
                        >
                            <div class="relative">
                                <div class="w-full h-32 bg-gradient-to-br from-amber-500/20 via-pink-500/20 to-purple-600/20 flex items-center justify-center backdrop-blur-sm">
                                    <div class="w-16 h-16 bg-gradient-to-r from-amber-400 to-pink-500 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-lg">
                                        📊
                                    </div>
                                </div>
                                <div class="absolute top-3 right-3">
                                    <span :class="`bg-${getEstadoColor(estado.nombre)}-500/20 text-${getEstadoColor(estado.nombre)}-400 border-${getEstadoColor(estado.nombre)}-500/30 px-3 py-1 rounded-full text-sm font-semibold border backdrop-blur-sm`">
                                        {{ estado.nombre }}
                                    </span>
                                </div>
                            </div>
                            <div class="p-6">
                                <h3 class="text-xl font-bold text-white mb-2">{{ estado.nombre }}</h3>
                                <p class="text-slate-400 mb-4">Estado de trabajo del sistema</p>
                                
                                <div class="flex items-center justify-between mb-4">
                                    <!--<span class="text-sm text-slate-400">ID: #{{ estado.id }}</span>-->
                                    <div class="flex items-center space-x-2">
                                        <Link :href="`/estados-trabajo/${estado.id}`" class="text-amber-400 hover:text-amber-300 transition-colors p-2 hover:bg-amber-500/20 rounded-lg">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                            </svg>
                                        </Link>
                                        <Link :href="`/estados-trabajo/${estado.id}/edit`" class="text-pink-400 hover:text-pink-300 transition-colors p-2 hover:bg-pink-500/20 rounded-lg">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </Link>
                                        <button @click="deleteEstado(estado.id)" class="text-red-400 hover:text-red-300 transition-colors p-2 hover:bg-red-500/20 rounded-lg">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="text-xs text-slate-500">
                                Creado: {{ formatDate(estado.created_at) }}
                            </div>
                        </div>
                    </div>
