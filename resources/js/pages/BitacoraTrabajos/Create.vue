    ...
    <AppContent>
      <div class="mb-8 bg-gradient-to-r from-slate-800/50 to-slate-900/50 backdrop-blur-lg border-b border-white/10">
        <div class="px-8 py-6 flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-cyan-300 via-sky-400 to-indigo-400 bg-clip-text text-transparent">Nuevo Registro de Bitácora</h1>
            <p class="text-slate-400">Registra una acción realizada sobre un trabajo</p>
          </div>
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input v-model="form.accion" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white focus:ring-2 focus:ring-cyan-500" placeholder="Ej: Creación, Actualización, Eliminación" required />
            ...
            <div>
              ...
              <textarea v-model="form.descripcion" rows="3" class="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-lg text-white focus:ring-2 focus:ring-cyan-500" placeholder="Detalles de la acción"></textarea>
            ...
            <div class="flex justify-end gap-4 pt-6 border-t border-white/10">
              ...
              <button type="submit" :disabled="form.processing" class="px-6 py-3 bg-gradient-to-r from-cyan-500 to-indigo-600 text-white rounded-lg hover:from-cyan-600 hover:to-indigo-700 transition">{{ form.processing ? 'Guardando...' : 'Guardar' }}</button>
            </div>
          </form>
        </div>
      </div>
...
const props = defineProps({ trabajos: { type: Array, required: true }, usuarios: { type: Array, required: true } })

const form = useForm({ idTrabajo: '', idUsuario: '', accion: '', fecha: '', descripcion: '' })
const submit = () => form.post(route('bitacora-trabajos.store'))
</script>
